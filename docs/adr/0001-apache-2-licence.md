...
FIDE only accepts full pairing programs, so this library gains recognition by being embedded in them, including closed commercial arbiter software; a copyleft licence (GPL, or LGPL, which is awkward for Java jars) would shut those programs out. We chose Apache-2.0 over MIT for its explicit patent grant, and because it keeps any port of logic from the Apache-2.0 oracle bbpPairings clean with a NOTICE attribution.
