...
The project enforces repository-specific testing rules. Treat these as higher-priority guidance for tests in this repository:

- Prefer handwritten fakes over Mockito-style mocks for in-repo components (repositories, domain objects, application services, value objects). Use mocks only for external systems or boundaries you do not control.
- Prefer stateful fakes to interaction-based mocks; assert observable state and behavior rather than interactions.
- Do not start the Spring context in unit tests. Wire dependencies manually (constructor injection); keep any test object-graph helper deterministic, with no logic.
- Use fixed clocks for time-dependent behavior.
- Never change production code to satisfy tests. Fix tests by providing required dependencies using factories and fakes.
- If a fake becomes complex, consider narrowing repository interfaces or extracting read/query behavior.
