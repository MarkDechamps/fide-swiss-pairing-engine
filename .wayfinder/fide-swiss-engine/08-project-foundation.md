...
- Carried out afterwards on the author's request: the research branches were merged into `main` (the research files now live in `docs/research/`), the Maven scaffold was built (poms, wrapper, `module-info`s, CI and nightly workflows, LICENSE, README), and the repo was pushed to GitHub.
