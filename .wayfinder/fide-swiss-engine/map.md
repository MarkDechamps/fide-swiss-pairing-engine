...
- Algorithm design for each remaining system (Swiss Team once its rulings are made, Dubov, Burstein, Lim, Double-Swiss), one prototype per system following the Dutch pattern (literal generator, criteria vector, matching oracle, optimum finder). Whether criteria get a shared abstraction is settled with the second one.
