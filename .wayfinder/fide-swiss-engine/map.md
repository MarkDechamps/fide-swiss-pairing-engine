...
- Algorithm design for each remaining system (Dubov, Burstein, Lim, Double-Swiss), one prototype per system following the Dutch pattern (literal generator, criteria that declare how they add up, one round-wide matching for target and reachability). Whether criteria get a shared abstraction is settled with the second one.
