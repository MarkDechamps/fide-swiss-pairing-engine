...
- TRF CLI surface (JaVaFo/bbp-compatible `-p`/`-check`; settings precedence is settled in Tournament profiles; the generator's own design is the Random tournament generator ticket), our team-pairing reply format, and how to handle what TRF26 can't express (Lim `192` code, XXS→162 losses).
