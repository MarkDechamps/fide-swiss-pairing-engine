...
| A6 [C6] | Interpretation `UpfloaterLookAhead`: **parity minimum**, pass/fail (default) or **graded** (fewest upfloaters the following scoregroup needs, then [C7], then the first set). Either way, the "following scoregroup" is the scoregroup right below the bracket's score in the round's standings; if all its teams are already paired, [C6] holds (corrected by Readable Swiss Team pairing algorithm: the earlier wording "next non-empty score level among the teams still to be paired" is not Gacrux's reading and changed 6 corpus rounds) | Interpretation | default = Gacrux; graded = `alt.py c6-graded` |
